...
| Should `Symbol / Sign / Representation` (one of the nine `0.3.0`-batch concepts, `docs/crosswalks/concepts/symbol-sign-representation.md`) be added to this core at all? | Its own Uncertainty section flags an unresolved question of whether the evidence (GFO symbol-structure types, content-bearing physicals/linguistic expressions, naming/signification relations) should become one class, a class plus relation(s), or multiple sibling concepts, and its own Future Work says this must be resolved "before any `core.ttl` placement decision." | Deliberately not added by this pass; revisit once the single-class-vs-family question is resolved. |
