...
**Reused for `time.md` (2026-07-04):** `TUpper-Terms.html` was re-fetched and its `timepoint` entry read directly (quoted above in full): "Timepoints form a linear ordering." Also confirmed `inf-`/`inf+` (the unique earliest/latest timepoint) and `beginof` (a function from an activity-occurrence to the timepoint at which it begins: "If x is an activity occurence... then (beginof x) has the value t if and only if t is the timepoint at which the activity occurrence x begins"). `timepoint` is TUpper's **only** temporal class — one of its four flat, co-equal top-level classes (`object`/`activity`/`activity_occurrence`/`timepoint`, `object` defined negatively as "anything that is not a timepoint, nor an activity nor an activity-occurrence"). No interval/duration-equivalent class exists in the fetched files; `timepoint` denotes an instant-like entity in a linear order, not a region. Note: the canonical `tupper.all.owl` path recorded above 404'd on this pass (repository structure may have shifted) — `tupper.clif`/`TUpper-Terms.html` are still live and were used instead; a future pass touching TUpper's OWL rendering specifically should re-verify the current path before citing it. See `time.md`'s Source Definitions and correspondence row 006.

**Reused for `docs/methodology/source-ontology-module-conventions.md` (2026-07-05, resolving `ADR-0021`'s TUpper gap — the strongest hit of all 8 sources checked):** the `gruninger/colore` repository's `ontologies` directory was fetched directly via the GitHub Contents API and found to contain a directory literally named `psl_core`, sibling to over a dozen `psl_*` extension directories (`psl_actocc`, `psl_actors`, `psl_atomic`, `psl_complex`, `psl_disc_state`, `psl_duration`, `psl_locations`, `psl_occtree`, `psl_occurrence`, `psl_participates`, `psl_resources`, `psl_soo`, `psl_subactivity`, `extended_psl`, `interval_psl`). `tupper.clif` itself (already quoted above) imports four of these extension modules (`disc_state`, `actocc`, `occupy_root`, `fount`), which in turn build on PSL-Core. PSL's own published documentation (Grüninger et al., "The Process Specification Language (PSL) Overview and Version 1.0 Specification," NIST) states this explicitly: "The PSL Ontology is a set of first-order theories organized into **PSL-Core** and a partially ordered set of extensions. All extensions within PSL are consistent extensions of PSL-Core, although not all extensions within PSL need be mutually consistent." This is the clearest and most explicit "core" self-description found across all 8 sources — a literal directory name plus an explicit prose statement, closely paralleling GFO's own README language ("parallel extensions... not all... consistent with each other").
